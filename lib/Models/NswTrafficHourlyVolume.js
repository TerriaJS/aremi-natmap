...
NswTrafficHourlyVolumeCatalogItem.prototype._load = function() {
  ...
  const query = encodeURIComponent(`
        SELECT
          ...
          CASE
            WHEN CARDINAL_DIRECTION_SEQ='0' THEN 'BOTH'
            WHEN CARDINAL_DIRECTION_SEQ='1' THEN 'NORTH'
            WHEN CARDINAL_DIRECTION_SEQ='3' THEN 'EAST'
            WHEN CARDINAL_DIRECTION_SEQ='5' THEN 'SOUTH'
            WHEN CARDINAL_DIRECTION_SEQ='7' THEN 'WEST'
            WHEN CARDINAL_DIRECTION_SEQ='9' THEN 'NORTHBOUND-SOUTHBOUND'
            WHEN CARDINAL_DIRECTION_SEQ='10' THEN 'EASTBOUND-WESTBOUND'
            ELSE ''
        ...
        FROM
            ...
            INNER JOIN LATERAL (
              SELECT STATION_KEY, DATE,CARDINAL_DIRECTION_SEQ,
              ${getHoursForSubQuery()}
              FROM road_traffic_counts_hourly_permanent
              WHERE YEAR='2019' AND DAY_OF_WEEK IN ('1','2','3','4','5')
              GROUP BY STATION_KEY,DATE,CARDINAL_DIRECTION_SEQ
            ) as SB ON REF.STATION_KEY=SB.STATION_KEY
        GROUP BY
            REF.STATION_ID,NAME,LGA,SUBURB,WGS84_LATITUDE,WGS84_LONGITUDE,ROAD_FUNCTIONAL_HIERARCHY,CARDINAL_DIRECTION_SEQ
  `);
